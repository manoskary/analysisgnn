...
class BuiltinDataset(StrutturaDataset):
    ...
    is_zip : bool
        Whether the downloaded file is a zip file. Default: False
    clone: bool
        Whether to clone the repository. Default: False
    branch: str
        The branch to clone. Default: None
    fetch_submodules: bool
        Whether to fetch submodules when cloning a git repository. Default: False
    """
    def __init__(self, name, url, raw_dir=None, hash_key=(), force_reload=False, verbose=False, is_zip=False, clone=False, branch=None, fetch_submodules=False):
        self.is_zip = is_zip
        ...
        if self.clone:
            ...
            self.branch = None
        self.fetch_submodules = fetch_submodules if self.clone else False
    ...
    def download(self):
        ...
        elif "https://github.com/" in self.url or self.clone or "https://gitlab." in self.url:
            if self.verbose:
                print(f"Cloning {self.name} from {self.url}...")
            repo_path = os.path.join(self.raw_dir, self.name)
            try:
                Repo.clone_from(self.url, repo_path, single_branch=True, b=self.branch, depth=1)
                if self.verbose:
                    print(f"Cloning successful for branch {self.branch}.")
            except git.GitCommandError:
                if self.verbose:
                    print(f"Cloning failed for branch {self.branch}. Switching to branch=`main` and trying again.")
                Repo.clone_from(self.url, repo_path, single_branch=True, b="main", depth=1)
