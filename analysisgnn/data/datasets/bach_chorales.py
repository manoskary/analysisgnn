...
from analysisgnn.utils.hgraph import *
from analysisgnn.utils.pianoroll import pianorolls_from_part
import os
from analysisgnn.data.dataset import BuiltinDataset, StrutturaDataset, get_download_dir
from joblib import Parallel, delayed
from tqdm import tqdm
from analysisgnn.models.core import positional_encoding
from analysisgnn.data.datasets.mcma import preprocess_na_to_monophonic, get_mcma_truth_edges, get_edges_mask
# Commented out due to removed vocsep dependency
# from analysisgnn.data.vocsep import GraphVoiceSeparationDataset
...
class Bach370ChoralesDataset(BuiltinDataset):
    ...
    def process(self):
        ...
        self.score_names = [os.path.splitext(os.path.basename(file))[0] for file in self.scores]
...
class Bach370ChoralesPitchSpellingGraphDataset(InMemoryDataset):
    def __init__(self, raw_dir=None, force_reload=False,
                 verbose=True, num_workers=4):
        self.base_dataset = Bach370ChoralesDataset(raw_dir=raw_dir)
        if verbose:
            print("Loaded Bach 370 Chorales Dataset Successfully, now processing...")
        root = os.path.join(get_download_dir(),
                            "Bach370ChoralesPitchSpellingGraphDataset") if raw_dir is None else os.path.join(
            raw_dir, "Bach370ChoralesPitchSpellingGraphDataset")
        ...
        self.test_pieces = []
        self.prob_pieces = []
        super(Bach370ChoralesPitchSpellingGraphDataset, self).__init__(root, transform, pre_filter)
...
# class Bach370ChoralesPGVoiceSeparationDataset(StrutturaDataset):
#     r"""The Bach 370 Chorales Graph Voice Separation Dataset for pytorch geometric.
