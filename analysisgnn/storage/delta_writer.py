...
_BINARY_TASK_VOCABULARIES: Dict[str, List[str]] = {
    "phrase": ["no_boundary", "phrase_end"],
    "section": ["no_section_start", "section_start"],
    "tpc_in_label": ["NCT", "chord_tone"],
    "tpc_is_root": ["not_root", "is_root"],
    "tpc_is_bass": ["not_bass", "is_bass"],
    "organ_point": ["no_pedal", "pedal_present"],
}
...
_EXTRA_REPRESENTATION_CLASSES: Dict[str, str] = {
    "note_degree": "NoteDegree49",
}


def resolve_task_vocabulary(task_name: str, num_classes: int) -> List[str]:
    ...
    if task_name in _EXTRA_REPRESENTATION_CLASSES:
        ...
        import analysisgnn.utils.chord_representations as cr
        cls = getattr(cr, cls_name, None)
...
def _class_list_to_strings(class_list: list) -> List[str]:
...
def _fit_vocab(vocab: List[str], num_classes: int, task_name: str) -> List[str]:
