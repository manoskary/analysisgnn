...
class RelEdgeConv2(nn.Module):
    ...
    def __init__(self, in_node_features, out_features, bias=True, in_edge_features=None):
        ...
        super(RelEdgeConv, self).__init__()
        self.W1 = nn.Linear(in_node_features, out_features, bias=bias)
        self.W2 = nn.Linear(in_node_features, out_features, bias=bias)
        self.W3 = nn.Linear(in_node_features, out_features, bias=bias)
        self.W4 = nn.Linear(in_node_features, out_features, bias=bias)
...
class ResGatedGraphConv(nn.Module):
    ...
    def forward(self, features, edge_index, edge_features=None, neigh_feats=None):
        ...
        h3 = self.W3(features)[edge_index[0]]
        h4 = self.W4(features)[edge_index[1]]
        if edge_features is not None and self.in_edge_features is not None:
            h5 = self.W5(edge_features)
            z1 = torch.sigmoid(h3 + h4 + h5)
        else:
...
class MetricalConvLayer(nn.Module):
    def __init__(self, in_dim, out_dim, activation=None, dropout=0.2, bias=True):
        ...
        self.conv_out = nn.Linear(4 * in_dim, out_dim, bias=bias)
        self.seq = nn.GRU(in_dim, in_dim, batch_first=True, bias=bias, bidirectional=True)
        # TODO add attention
    ...
    def forward(self, x_metrical, x, edge_index, lengths):
        if lengths is None:
            lengths = torch.tensor([x_metrical.size(0)], dtype=torch.long).to(x_metrical.device)
        use_padding = not torch.all(lengths == lengths[0])
        ...
        if use_padding:
            z_s = torch.cat((h_scatter, x_metrical), dim=-1)
            z_s = torch.split(z_s, torch.diff(lengths).tolist())
            z_s = pad_sequence(z_s, batch_first=True)
            h_metrical = torch.split(h_scatter, torch.diff(lengths).tolist())
            h_metrical = pad_sequence(h_metrical, batch_first=True)
        else:
            h_metrical = h_scatter.view(-1, lengths[0], h_scatter.shape[1])
            z_s = torch.cat((h_scatter, x_metrical), dim=-1)
            z_s = z_s.view(-1, lengths[0], z_s.shape[1])

        h_seq = self.seq(h_metrical)[0]
        h = torch.cat([z_s, h_seq], dim=-1)

        h = self.conv_out(h)
        h = self.activation(h)
        h = torch.einsum('ijk->ikj', h)
        h = self.normalize(h)
        h = self.dropout(h)
        h = torch.einsum('ijk->ikj', h)
        if use_padding:
            # Flatten padded sequence without pad values
            mask = torch.arange(h.shape[1]).unsqueeze(0).repeat(h.shape[0], 1).to(h.device) < torch.diff(
                lengths).unsqueeze(1)
            h = h[mask].view(-1, h.shape[-1])
        else:
