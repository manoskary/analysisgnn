...
from analysisgnn.models.core import *
from torch_scatter import scatter_add
from torch_sparse import coalesce
from pytorch_lightning import LightningModule
from analysisgnn.metrics.eval import MultitaskAccuracy
import pandas as pd
from analysisgnn.models.core.hgnn import MetricalGNN
from analysisgnn.utils import add_reverse_edges_from_edge_index
...
class OnsetEdgePooling(nn.Module):
    def __init__(self, in_channels, dropout=0, add_to_edge_score=0.5):
        super(OnsetEdgePooling, self).__init__()
    ...
    def forward(self, x, edge_index, batch):
        ...
        x, edge_index, batch = self.__merge_edges__(h, edge_index, batch, e)
...
class HybridChordEncoder(nn.Module):
    def __init__(self, in_feats, n_hidden, n_layers, metadata, spelling_dim=49, dropout=0.5, **kwargs):
        super(HybridChordEncoder, self).__init__()
        self.spelling_embedding = nn.Embedding(spelling_dim, 128)
        self.gnn = HybridGNN(edge_types=metadata[1], input_channels=128, hidden_channels=n_hidden, num_layers=n_layers, dropout=dropout, **kwargs)
        self.x_map = nn.ModuleDict()
        self.x_map["note"] = nn.Linear(in_feats+128, 128)
        for node_type in metadata[0]:
            if node_type != "note":
                self.x_map[node_type] = nn.Linear(in_feats, 128)

    def forward(self, pitch_spelling, x_dict, edge_index_dict, batch_dict, num_sampled_nodes_dict, num_sampled_edges_dict, batch_size):
        h_spelling = self.spelling_embedding(pitch_spelling)
        x_dict["note"] = torch.hstack((x_dict["note"], h_spelling))
        x_dict = {k: self.x_map[k](v) for k, v in x_dict.items()}
        h = self.gnn(x_dict, edge_index_dict, batch_dict, batch_size, num_sampled_nodes_dict, num_sampled_edges_dict)
        return h


class MetricalChordPredictionModel(nn.Module):
    def __init__(self, in_feats, n_hidden=256, pitch_dims=35, tasks: dict = {
        ...
        self.n_layers = n_layers
        self.tasks = tasks
        self.pitch_embedding = nn.Embedding(35, 128)
        self.encoder = MetricalChordEncoder(in_feats+128, n_hidden, n_layers, activation=activation, dropout=dropout,
...
class BasicChordPredictionModel(nn.Module):
    def __init__(self, metadata, in_feats, n_hidden=256, spelling_dim=35, tasks: dict = {
        "localkey": 38, "tonkey": 38, "degree1": 22, "degree2": 22, "quality": 11, "inversion": 4,
        "root": 35, "romanNumeral": 31, "hrhythm": 7, "pcset": 121, "bass": 35, "tenor": 35,
        "alto": 35, "soprano": 35}, n_layers=1, activation=F.relu, dropout=0.5, linear_probing=False):
        super(BasicChordPredictionModel, self).__init__()
        self.dropout = dropout
        self.n_layers = n_layers
        self.tasks = tasks
        self.encoder = HybridChordEncoder(in_feats, n_hidden, n_layers, metadata=metadata, spelling_dim=spelling_dim, dropout=dropout)
        self.classifier = MultiTaskMLP(n_hidden, n_hidden, tasks=tasks, n_layers=2, activation=activation, dropout=dropout)
        if linear_probing:
            self.freeze_encoder()
    ...
    def forward(self, pitch_spelling, x_dict, edge_index_dict, batch_dict, num_sampled_nodes_dict, num_sampled_edges_dict, batch_size):
        ...
        pitch_spelling: torch.Tensor
            (n_nodes, )
        x_dict: dict
            Dictionary with node features, i.e. note, beat, measure, etc.
        edge_index_dict: dict
            Dictionary with edge indices.
        batch_dict: dict
            Dictionary with batch vectors.
        num_sampled_nodes_dict: dict
            Dictionary with number of sampled nodes.
        num_sampled_edges_dict: dict
            Dictionary with number of sampled edges.
        batch_size: int
            Batch size. Number of target nodes.

        """
        h = self.encoder(pitch_spelling, x_dict, edge_index_dict, batch_dict, num_sampled_nodes_dict, num_sampled_edges_dict, batch_size)
...
class ChordPredictionModel(nn.Module):
    ...
    def predict(self, score):
        from analysisgnn.utils import hetero_graph_from_note_array, select_features, add_reverse_edges_from_edge_index
        ...
        nodes, edges = hetero_graph_from_note_array(note_array=note_array, pot_edge_dist=0)
...
class ChordPrediction(LightningModule):
    def __init__(self,
        ...
        if self.use_rotograd:
            raise NotImplementedError("Rotograd not implemented yet")
...
class MetricalChordPrediction(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        na = batch["note_array"]
        edge_features = F.normalize(
            torch.abs(batch["note_array"][edges[0]] - batch["note_array"][edges[1]]),
...
class ChordPredictionPLModel(LightningModule):
    def __init__(self,
                 in_feats: int,
                 n_hidden: int,
                 tasks: dict,
        ...
        super(ChordPredictionPLModel, self).__init__()
        self.tasks = tasks
        self.save_hyperparameters()
        self.num_tasks = len(tasks.keys())
        self.module = BasicChordPredictionModel(
            metadata=metadata,
            in_feats=in_feats,
            n_hidden=n_hidden,
            tasks=tasks,
            spelling_dim=spelling_dim,
            n_layers=n_layers,
            dropout=dropout,
            linear_probing=kwargs.get("linear_probing", False),
        )
        self.lr = lr
        self.weight_decay = weight_decay
        self.test_roman = list()
        self.test_roman_ts = list()
        self.train_loss = MultiTaskLoss(
            list(tasks.keys()), nn.ModuleDict({task: nn.CrossEntropyLoss(ignore_index=-1) for task in tasks.keys()}), requires_grad=True)
        self.val_loss = MultiTaskLoss(
            list(tasks.keys()), nn.ModuleDict({task: nn.CrossEntropyLoss() for task in tasks.keys()}), requires_grad=False)
        self.test_acc = MultitaskAccuracy(tasks)
    ...
    def training_step(self, batch, batch_idx):
        ...
        num_sampled_nodes_dict = graph.num_sampled_nodes_dict
        ...
        local_key = (batch_pred["localkey"].argmax(dim=1) == batch_labels["localkey"]).unsqueeze(0)
        acc_RomNum = (torch.cat((degree, quality, root, inversion, local_key), dim=0).sum(0) == 5).float().mean()
        self.log('Train/RomNum', acc_RomNum.item(), prog_bar=True, batch_size=batch_size)
        self.log("global/step", self.global_step, on_step=True, prog_bar=False, batch_size=batch_size)
        return loss["total"]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        num_sampled_edges_dict = graph.num_sampled_edges_dict
        batch_labels = {k: graph["note"][k][:batch_size] for k in self.tasks.keys()}
        batch_pred = self.module(pitch_spelling, x_dict, edge_index_dict, batch_dict, num_sampled_nodes_dict,
                                 num_sampled_edges_dict, batch_size)
        loss = self.val_loss(batch_pred, batch_labels)
        self.log('val/total_loss', loss["total"].item(), prog_bar=True, batch_size=1)
        degree = torch.logical_and(
            batch_pred["degree1"].argmax(dim=1) == batch_labels["degree1"],
            batch_pred["degree2"].argmax(dim=1) == batch_labels["degree2"]).unsqueeze(0)
        root = (batch_pred["root"].argmax(dim=1) == batch_labels["root"]).unsqueeze(0)
