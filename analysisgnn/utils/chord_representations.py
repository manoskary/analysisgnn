...
def create_data(filtered_df, time_signature, interval="P1"):
    ...
    localkey = encode_one_hot(filtered_df, LocalKey50, transposition=interval)
    tonkey = encode_one_hot(filtered_df, TonicizedKey50, transposition=interval)
    quality = encode_one_hot(filtered_df, ChordQuality15, transposition=interval)
    root = encode_one_hot(filtered_df, ChordRoot38, transposition=interval)
    ...
    bass = encode_one_hot(filtered_df, Bass38, transposition=interval)
...
def time_divided_tsv_to_note_array(time_divided_tsv_path, transpose=False, version="v1.0.0", include_measures=False):
    ...
    measure_end_beats = np.r_[measure_start_beats[1:], offset[-1] // 1 + 1 ]
    ...
    label_names = ["localkey", "tonkey", "degree1", "degree2", "quality", "inversion", "root", "romanNumeral", "hrythm", "pcset", "bass", "onset"]
    if transpose:
        ...
        for interval in transpositions:
            ...
            data.append((X, y, np.vstack((measure_start_beats, measure_end_beats)).T, label_names))
        return data
    else:
        X, y = createfunction(filtered_df, time_signature)
        return X, y, np.vstack((measure_start_beats, measure_end_beats)).T, label_names
...
def time_divided_tsv_to_part(time_divided_tsv_path, transpose=False, version="v1.0.0"):
    ...
    if transpose:
        ...
        for (note_array, labels, measures, label_names) in X:
            ...
            new_X.append((note_array, labels, measures, label_names))
        return new_X
    else:
        note_array, labels, measures, label_names = time_divided_tsv_to_note_array(time_divided_tsv_path, transpose=transpose, version=version)
        note_array, labels = tie_consecutive_notes(note_array, labels)
        note_array, divs = create_divs_from_beats(note_array)
        return note_array, labels, measures*divs, label_names
...
def _getTranspositions(df):
    ...
    for interval in INTERVALCLASSES:
        ...
        if set(transposed).issubset(set(M21_KEYS)):
            ret.append(interval)
    return ret
...
class SimpleRomanNumeral185(OutputRepresentationTI):
    classList = SIMPLE_NUMERAL_VOCABULARY
    dfFeature = "a_simpleNumeral"


class PrimaryDegree22(OutputRepresentationTI):
    classList = M21_DEGREES
    dfFeature = "a_degree1"
...
class NoteDegree49(OutputRepresentationTI):
    classList = NOTE_DEGREES
    dfFeature = "note_degree"


class LocalKey50(OutputRepresentation):
    classList = list(EMPIRICAL_KEYS.values())
    dfFeature = "a_localKey"
    transpositionFn = staticmethod(TransposeKey)


class TonicizedKey50(OutputRepresentation):
    classList = list(EMPIRICAL_KEYS.values())
    dfFeature = "a_tonicizedKey"
    transpositionFn = staticmethod(TransposeKey)


class ChordRoot38(OutputRepresentation):
    classList = list(EMPIRICAL_TONE_FUNCTIONS.values())
    dfFeature = "a_root"
    transpositionFn = staticmethod(TransposePitch)


class Bass38(OutputRepresentation):
    classList = list(EMPIRICAL_TONE_FUNCTIONS.values())
    dfFeature = "a_bass"
    transpositionFn = staticmethod(TransposePitch)
