...
from analysisgnn.utils.general import exit_after, MapDict
# from analysisgnn.utils.graph import ScoreGraph
from analysisgnn.descriptors.general import *
import torch
...
class BatchedHeteroScoreGraph(HeteroScoreGraph):
    def __init__(self, note_features, edges, lengths, etypes=["onset", "consecutive", "during", "rest"], names=None, note_array=None, edge_weights=None, labels=None):
        super(BatchedHeteroScoreGraph, self).__init__(note_features, edges, etypes, names, note_array, edge_weights, labels)
        self.lengths = lengths

    def unbatch(self):
        graphs = []
        for i, l in enumerate(self.lengths):
            graphs.append(HeteroScoreGraph(self.x[l:self.lengths[i+1]], self.edge_index[:, l:self.lengths[i+1]], self.etypes, self.name[i], self.note_array[i, :l], self.edge_weights[i, :l], self.y[i]))
        return graphs
...
def batch_graphs(graphs):
    ...
    batched_graph: HeteroScoreGraph
        ...
        batched attributes.
    """
    lengths = np.array([0] + [len(g.x) for g in graphs[:-1]], dtype=int)
...
def edge_index_to_dict(edge_index, edge_types, etypes):
    edge_dict = {}
    for etype, evalue in etypes.items():
        mask = torch.zeros(edge_types.shape[0], dtype=bool, device=edge_types.device)
        torch.where(edge_types == evalue, True, False, out=mask)
        edge_dict[etype] = edge_index[:, mask]
    return edge_dict
