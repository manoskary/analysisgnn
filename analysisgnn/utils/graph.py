...
from analysisgnn.utils.hgraph import HeteroScoreGraph
...
def score_graph_to_pyg(
    ...
    #     if key not in ["x", "edge_index", "y", "edge_attr", "edge_weights", "node_features", "etypes",  ]:	
...
def node_subgraph(node_idx, edge_index, num_nodes):
    node_mask = torch.zeros(num_nodes, dtype=torch.long, device=edge_index.device)
